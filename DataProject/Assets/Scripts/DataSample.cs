...
public class DataSample : MonoBehaviour
{
    ...
    //게임 여부에 따라 플레이 할 때 기존의 정보를 가지고 진행하는 게임도 존재

    //PlayerPrefs는 주로 플레이어에 대한 환경 설정을 저장할 때 사용되는 클래스
    //해당 클래스는 문자열, 실수, 정수를 사용자의 플랫폼 레지스토리에 저장할 수 있음

    //PlayerPrefs는 Key와 Value로 구성되어 있는 데이터.(C#의 Dictionary)
    //Key는 Value에 접근하기 위한 데이터(실제 데이터의 위치)
    //Value는 Key를 통해 접근할 수 있는 실질적인 값

    //ex) userID : current147으로 저장되어 있다면, userID가 Key, current147이 Value에 해당
   
    public UserData userData;
    //1.유니티 에디터에서 따로 userData에 대한 정보를 더 작성할 것
    //2. 레지스트리에 있는 키 값을 검색
    //3. 키 전체 삭제

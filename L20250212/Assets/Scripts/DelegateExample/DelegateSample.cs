...
{
    //델리게이트(delegate)
    //함수를 대입할 수 있는 변수, 일반적으로 대리자라고 부름
    //해당 변수에는 함수가 대입되어 있으므로, 해당 변수를 실행하면 대입한 함수가 실행되는 방식을 가지고 있음.
    ...
    void Start()
    {
        ...
        //일반적인 함수 호출이 아닌 delegate변수를 만들어서 객체 할당하는 이유?
        //1. delegate는 함수가 아닌 타입이기 때문
        //   매개변수로도 활용이 가능하고, return타입으로 잡아주는 것도 가능


        //2. 델리게이트 체인(delegate Chain)
        //delegate는 +=을 통해 대리할 함수를 더 추가할 수 있고, -=를 통해 대리한 함수를 제거하는 것도 가능
        dt += Attack;
        dt += Attack;
        dt += Attack;
        dt += Guard;
    ...
    void MoveLeft() => Debug.Log("왼쪽으로 이동");
